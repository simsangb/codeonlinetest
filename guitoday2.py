...
layout = [[sg.Text('선택 →'),sg.Radio('평일','dday', key ='-normalday-',default=True),sg.Radio('휴일','dday', key='-holiday-')],
          [sg.Text('선택 →'),sg.Radio('겨울철(12월~2월)','season', key ='-wwinter-',default=True),sg.Radio('간헐난방(3,4,11?월)','season', key='-sspring-'),sg.Radio('급탕만(5월~10월)','season', key='-ssummer-')],
          ...
          [sg.Button('ok',bind_return_key=True),sg.Button('reset'),sg.Button('cancel')],
...
window = sg.Window('열공급량 추정(참고용)',layout)

def plot():
    global figure_agg
    # fig = plt.figure(figsize=(12,4))
     # insert plot to fig code here
    if figure_agg:
          figure_agg.get_tk_widget().forget()
    figure_agg = draw_figure(window['-CANVAS-'].TKCanvas, create_plot(xtemp,ysupply))
...
while True:
    ...
    if (values['-normalday-'] == True) and (values['-wwinter-'] == True):

        new_model = MLPModel() # Model을 생성해줍니다.
        # print("this is test")
        new_model.load_state_dict(torch.load('winternormal.pt'))
        new_model.eval()
    if (values['-normalday-'] == True) and (values['-sspring-'] ==True):

        new_model = MLPModel() # Model을 생성해줍니다.
        new_model.load_state_dict(torch.load('15lownormal.pt'))
        new_model.eval()
    if (values['-normalday-'] == True) and (values['-ssummer-'] ==True):

        new_model = MLPModel() # Model을 생성해줍니다.
        new_model.load_state_dict(torch.load('15overnormal.pt'))
        new_model.eval()
    if (values['-holiday-'] == True) and (values['-wwinter-'] == True):

        new_model = MLPModel() # Model을 생성해줍니다.
        new_model.load_state_dict(torch.load('winterholiday.pt'))
        new_model.eval()
    if (values['-holiday-'] == True) and (values['-sspring-'] ==True):

        new_model = MLPModel() # Model을 생성해줍니다.
        new_model.load_state_dict(torch.load('15lowhol.pt'))
        new_model.eval()
    if (values['-holiday-'] == True) and (values['-ssummer-'] ==True):

        new_model = MLPModel() # Model을 생성해줍니다.
        new_model.load_state_dict(torch.load('15overhol.pt'))
        new_model.eval()
    ...
    if event == 'reset':
        ...
        window['-in8-'].update("0")
        window['-in9-'].update("0")
        window['-in10-'].update("0")
        window['-in11-'].update("0")
        window['-in12-'].update("0")
        window['-in13-'].update("0")
        window['-in14-'].update("0")
        window['-in15-'].update("0")
        window['-in16-'].update("0")
        window['-in17-'].update("0")
        window['-in18-'].update("0")
        window['-in19-'].update("0")
        window['-in20-'].update("0")
        window['-in21-'].update("0")
        window['-in22-'].update("0")
        window['-in23-'].update("0")
    ...
    if event == 'ok':
        ...
        if (values['-normalday-'] == True) and (values['-sspring-'] == True):
            ttlist1 = [-1.384048,	-1.249847,	-1.115645,	-0.981443,	-0.847242,	-0.71304,	-0.578839,	-0.444637,	-0.310435,	-0.176234,	-0.042032,	0.09217,	0.226371,	0.360573,	0.494775,	0.628976,	0.763178,	0.89738,	1.031581,	1.165783,	1.299985,	1.434186,	1.568388,	1.70259]
        if (values['-holiday-'] == True) and (values['-sspring-'] == True):
            ttlist1 = [-1.661232,	-1.516709,	-1.372187,	-1.227664,	-1.083141,	-0.938619,	-0.794096,	-0.649573,	-0.505051,	-0.360528,	-0.216006,	-0.071483,	0.07304,	0.217562,	0.362085,	0.506608,	0.65113,	0.795653,	0.940176,	1.084698,	1.229221,	1.373744,	1.518266,	1.662789]
        if (values['-normalday-'] == True) and (values['-ssummer-'] == True):
            ttlist1 = [-1.82473,	-1.67316,	-1.521589,	-1.370019,	-1.218449,	-1.066879	,-0.915309,	-0.763738,	-0.612168,	-0.460598,	-0.309028,	-0.157458,	-0.005887,	0.145683,	0.297253,	0.448823,	0.600394,	0.751964,	0.903534,	1.055104,	1.206674,	1.358245,	1.509815,	1.661385]
        if (values['-holiday-'] == True) and (values['-ssummer-'] == True):
            ttlist1 = [-1.66343,	-1.518918,	-1.374406,	-1.229894,	-1.085382,	-0.94087,	-0.796358,	-0.651846,	-0.507333,	-0.362821,	-0.218309,	-0.073797,	0.070715,	0.215227,	0.359739,	0.504251,	0.648763,	0.793275,	0.937787,	1.082299,	1.226811,	1.371323,	1.515835,	1.660347]
